...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False, args=None, logger=None):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 200, header, logger=logger):
        samples = samples.to(device)
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            if need_tgt_for_training:
                # outputs, _ = model(samples, dn_args=(args.num_patterns, args.device))
...
@torch.no_grad()
def accuracy(model, criterion, postprocessors, data_loader, base_ds, device, batch, wo_class_error=False, args=None, logger=None):
    try:
        need_tgt_for_training = args.use_dn
    except:
        need_tgt_for_training = False
    ...
    batch_count = 0  # 评测指定个batch

    for samples, targets in metric_logger.log_every(data_loader, 200, header, logger=logger):
        if batch_count == batch: break
        samples = samples.to(device)
        # import ipdb; ipdb.set_trace()
        # targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        targets = [{k: to_device(v, device) for k, v in t.items()} for t in targets]
