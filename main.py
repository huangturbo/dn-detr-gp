...
from util.utils import clean_state_dict, load_partial_weight_dab, transplant_weight_dab
...
def get_args_parser():
    ...
    parser.add_argument('--coco_path', type=str, default='/mnt/public/datasets/coco')
    ...
    parser.add_argument('--resume', default='', help='resume from checkpoint') # TODO
...
def main(args):
    ...
    # TODO
    # args.cfgs = [[[160, 64], [176, 704], [256, 832], [240, 1408], [208, 1984], [240, 2048]],
    # [[16, 112, 896], [128, 128, 1408], [240, 160, 1088], [192, 144, 1728], [224, 144, 1408], [240, 144, 448]]]
    ...
    if args.cfgs is None:
        # parameter transplant===================================================
        print("Param transplant start...")
        # 计算剩余通道、mask列表
        encoder_cfg = []
        encoder_cfg_mask = []
        decoder_cfg = []
        ...
        # attn = [14, 26, 38, 50, 62, 74, 108, 136, 164, 192, 220, 248, 109, 137, 165, 193, 221, 249]  # attn层
        # ffn = [15, 27, 39, 51, 63, 75, 110, 138, 166, 194, 222, 250] # ffn层
        for k, m in enumerate(model_without_ddp.modules()):
            if isinstance(m, channel_selection):
                if k in [14, 26, 38, 50, 62, 74]:  # encoder
                    weight_copy = m.prune_a.cpu().data.clone()
                    mask = weight_copy[0, 0, :].gt(0).float().to(device)
                    encoder_cfg.append(int(torch.sum(mask)))  # 剩余通道
                    encoder_cfg_mask.append(mask.clone())  # mask列表
                    print('layer index: {:d} \t total channel: {:d} \t remaining channel: {:d}'.
                      format(k, mask.shape[0], int(torch.sum(mask))))
                elif k in [15, 27, 39, 51, 63, 75]:  # encoder
                    weight_copy = m.prune_a.cpu().data.clone()
                    mask = weight_copy[0, 0, :].gt(0).float().to(device)
                    encoder_cfg.append(int(torch.sum(mask)))  # 剩余通道
                    encoder_cfg_mask.append(mask.clone())  # mask列表
                    print('layer index: {:d} \t total channel: {:d} \t remaining channel: {:d}'.
                      format(k, mask.shape[0], int(torch.sum(mask))))
                elif k in [108, 136, 164, 192, 220, 248, 109, 137, 165, 193, 221, 249]: # decoder
                    weight_copy = m.prune_a.cpu().data.clone()
                    mask = weight_copy[0, 0, :].gt(0).float().to(device)
                    ...
                    print('layer index: {:d} \t total channel: {:d} \t remaining channel: {:d}'.
                      format(k, mask.shape[0], int(torch.sum(mask))))
                elif k in [110, 138, 166, 194, 222, 250]:  # decoder
                    weight_copy = m.prune_a.cpu().data.clone()
                    mask = weight_copy[0, 0, :].gt(0).float().to(device)
                    decoder_cfg.append(int(torch.sum(mask)))  # 剩余通道
                    decoder_cfg_mask.append(mask.clone())  # mask列表
        ...
        # torch.save({'trans_state_dict': new_model_without_ddp.state_dict()}, # TODO
        #            os.path.join(args.output_dir, 'detr_prune_attn_trans.pth'))
        ...
        lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, args.lr_drop)
        args.start_epoch = 0
        print("Param transplant end...")
        model = new_model
        model_without_ddp = new_model_without_ddp
        # test_stats, coco_evaluator = evaluate(model, criterion, postprocessors,
        #                                       data_loader_val, base_ds, device, args.output_dir, wo_class_error=wo_class_error, args=args)
        # 参数移植结束===================================================
    # finetune
