...
def get_args_parser():
    ...
    parser.add_argument('--map_tol', default=0.02, type=float,
                        help='tol to stop pruning a layer. Larger tol means more neurons to prune')
    parser.add_argument('--skip_eval_converge', default=0.0, type=float,
...
def decide_candidate_attn(net, eval_train_loader, m, candidate_plus, criterion, isfirst=False):
    ...
    if isfirst:
        for i, item in enumerate(topk):
            ...
            args.cfgs_mask.append(opt_index)
        # print("opt_index: {}".format(args.cfgs_mask))
    else:
        for i, item in enumerate(topk):
            opt_index = next(iter(item))
            opt_stepsize = 1. / (current_num_neuron + i + 1)
...
def prune_a_layer(m, k, net, eval_train_loader, criterion, postprocessors, base_ds, layer_type, cnt_layer):
    ...
    while True:
        ...
        # selection section was used to evaluate all neurons
        ...
        if batch_map >= (1. - args.skip_eval_converge) * init_map:
            ...
            if verbose:
                ...
                print(
                    'Layer: ({:d}); Cur Loss: {:.4f}; Cur map: ({:.4f}%); Init map: {:.4f}'.format(
                        num_layer, cur_loss, cur_map, init_map))
...
def net_prune(net, eval_train_loader, criterion, postprocessors, base_ds):
    ...
    for k, m in enumerate(net.modules()):
        if isinstance(m, channel_selection):
            ...
            # ====================Skip the parts that have been pruned
            args.cfgs_mask = []
            layer_type = "attn" if k in attn else "ffn"  # attn和ffn进入不同的剪枝层
            print("prune layer:{},layer type:{}".format(k, layer_type))
            a_para, a_num, global_cur_top1, isalladd = prune_a_layer(m, k, net, eval_train_loader, criterion,
