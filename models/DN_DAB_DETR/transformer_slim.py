...
#
#     def forward(self, x): # x(1064,2,256)
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=300, num_encoder_layers=6,
        ...
        self.encoder = TransformerEncoder(num_encoder_layers, cfgs[0], d_model,
                                          nhead, dropout, activation, normalize_before,
                                          encoder_norm)
        ...
        self.decoder = TransformerDecoder(num_decoder_layers, cfgs[1], nhead,
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, num_layers, encoder_cfg, d_model, nhead,
                 dropout, activation, normalize_before,
                 norm=None):
        super().__init__()
        ...
        for cfg in encoder_cfg:
            encoder_layer = TransformerEncoderLayer(d_model, nhead, cfg, dropout=dropout,
                                                    activation=activation, normalize_before=normalize_before)
            self.layers.append(encoder_layer)

        self.num_layers = num_layers
        self.query_scale = MLP(d_model, d_model, d_model, 2) # TODO
        self.norm = norm
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, num_layers, decoder_cfg, nhead, dropout, activation, normalize_before,
        ...
        # 根据剪枝后通道构造decoder层
        self.layers = nn.ModuleList([])
        for cfg in decoder_cfg:
            decoder_layer = TransformerDecoderLayer(d_model, nhead, cfg, dropout=dropout,
                                                    activation=activation, normalize_before=normalize_before)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, cfg=None, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False):
        super().__init__()
        # self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        # 重新实现self_attn
        self.in_proj_weight = nn.Parameter(torch.empty((3 * cfg[0], d_model)))
        self.in_proj_bias = nn.Parameter(torch.empty(3 * cfg[0]))

        self.out_proj = nn.Linear(cfg[0], d_model)

        # Implementation of Feedforward model
        self.linear1 = nn.Linear(d_model, cfg[1])
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(cfg[1], d_model)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, cfg=None, dropout=0.1,
                 activation="relu", normalize_before=False, keep_query_pos=False,
                 rm_self_attn_decoder=False):
        super().__init__()
        # Decoder Self-Attention
        if not rm_self_attn_decoder:
            self.sa_qcontent_proj = nn.Linear(d_model, cfg[0])
            self.sa_qpos_proj = nn.Linear(d_model, cfg[0])
            self.sa_kcontent_proj = nn.Linear(d_model, cfg[0])
            self.sa_kpos_proj = nn.Linear(d_model, cfg[0])
            self.sa_v_proj = nn.Linear(d_model, cfg[0])
            self.self_attn = MultiheadAttention(cfg[0], nhead, dropout=dropout, vdim=d_model, is_cross=False)

            self.norm1 = nn.LayerNorm(d_model)
            self.dropout1 = nn.Dropout(dropout)

        # Decoder Cross-Attention
        self.ca_qcontent_proj = nn.Linear(d_model, cfg[1])
        self.ca_qpos_proj = nn.Linear(d_model, cfg[1])
        self.ca_kcontent_proj = nn.Linear(d_model, cfg[1])
        self.ca_kpos_proj = nn.Linear(d_model, cfg[1])
        self.ca_v_proj = nn.Linear(d_model, cfg[1])
        self.ca_qpos_sine_proj = nn.Linear(d_model, cfg[1])
        self.cross_attn = MultiheadAttention(cfg[1]*2, nhead, dropout=dropout, vdim=d_model, is_cross=True)

        self.nhead = nhead
        self.rm_self_attn_decoder = rm_self_attn_decoder

        # Implementation of Feedforward model
        self.linear1 = nn.Linear(d_model, cfg[2])
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(cfg[2], d_model)
    ...
    def forward(self, tgt, memory,
        ...
        if not self.rm_self_attn_decoder:
            ...
            tgt2 = self.self_attn(q, k, value=v, attn_mask=tgt_mask,
                                key_padding_mask=tgt_key_padding_mask, is_cross=False)[0]
...
def build_transformer_cfg(args):
    return Transformer(
