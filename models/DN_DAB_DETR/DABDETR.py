...
from .transformer_slim import build_transformer_cfg
...
class DABDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
                    aux_loss=False, 
                    iter_update=True, # bbox坐标迭代机制
                    query_dim=4, # 完整的xywh4个值
                    bbox_embed_diff_each_layer=False, # 预测头是不是共享权重值
                    random_refpoints_xy=False, # # 随机初始化xy值，然后固定，论文附录中有一个这个实验
                    ):
        ...
        self.label_enc = nn.Embedding(num_classes + 1, hidden_dim - 1) # (92,255)
        self.num_classes = num_classes

        if bbox_embed_diff_each_layer: # 每个层有自己的bbox头
            self.bbox_embed = nn.ModuleList([MLP(hidden_dim, hidden_dim, 4, 3) for i in range(6)])
        else: # 只有一个头，所有层都是用相同的头
        ...
        # # detr中的query_embed(100,256)的作用 这里(300,4)
        ...
        if random_refpoints_xy:
            ...
            # xy并不会跟随网络的变化而更新，但是在具体的经历了各个decoder之后，还是会逐层更新的
            self.refpoint_embed.weight.data[:, :2].requires_grad = False

        # 图像特征到transformer内的投影(原始detr也有)
    ...
    def forward(self, samples: NestedTensor, dn_args=None):
        ...
        input_query_label, input_query_bbox, attn_mask, mask_dict = \
            prepare_for_dn(dn_args, embedweight, src.size(0), self.training, self.num_queries, self.num_classes,
                           self.hidden_dim, self.label_enc) # input_query_bbox(360,4,4) input_query_label(360,4,256)
        # 第二个返回值是参考点
...
def build_DABDETR(args):
    ...
    if args.cfgs == None:
        ...
        transformer = build_transformer_cfg(args)
